  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return PageFrame(
      ...
      action: PopupMenuButton<String>(
        ...
        onSelected: (value) =>
            value == 'camera' ? _camera(context, ref) : _file(context, ref),
        itemBuilder: (_) => const [
          PopupMenuItem(
            value: 'camera',
            child: ListTile(
              leading: Icon(Icons.camera_alt_outlined),
              title: Text('Fotografieren'),
            ),
          ),
          PopupMenuItem(
            value: 'file',
  ...
  Future<void> _camera(BuildContext context, WidgetRef ref) async {
    ...
    if (image != null && context.mounted) {
      await _save(
        ...
        ref,
        image.path,
        'Foto ${DateTime.now().day}.${DateTime.now().month}.${DateTime.now().year}',
        recognizeText: true,
      );
    }
  }
  ...
  Future<void> _file(BuildContext context, WidgetRef ref) async {
    ...
    if (path != null && context.mounted) {
      await _save(
        ...
        recognizeText:
            RegExp(r'\.(jpe?g|png)$', caseSensitive: false).hasMatch(path),
      );
    }
  }
  ...
  Future<void> _save(
    ...
    if (accepted == true && title.text.trim().isNotEmpty) {
      ...
      await ref
          ...
          .addDocument(path, title.text.trim(), ocrText: ocrText);
    }
  }
}
          ...
          child: Center(
            child: Column(
              children: [
                ...
                const Text(
                  'Fotografiere einen Arztbrief oder importiere PDF- und Bilddateien.',
